...
public class UniqueCard : Card
{
    ...
    public UniqueCard(string name, PatronId deck, CardId commonId, int cost, CardType type, int hp, ComplexEffect?[] effects, int hash, CardId? family, bool taunt, UniqueId uniqueId)
        : base(name, deck, commonId, cost, type, hp, effects, hash, family, taunt)
    {
        UniqueId = uniqueId;
